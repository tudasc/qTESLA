...
#pragma omp parallel default(none) private(nonce) shared(e, randomness_extended)
    {
...
#pragma omp for
        for (k = 0; k < PARAM_K; k++) {
            do {  // Sample the error polynomials
                ...
                ctr_keygen++;
#endif
                sample_gauss_poly(&e[k * PARAM_N], &randomness_extended[k * CRYPTO_SEEDBYTES],
                                  ++nonce);
...
{
#define NUMT 64
    unsigned char c[NUMT][CRYPTO_C_BYTES];
    poly z[NUMT];
    poly_k a;
    int succthread = -1;
...
omp_set_num_threads(VarManager::instance().getThreadNum());
