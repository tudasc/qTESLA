...
public class InnerSigningThread extends Thread  {
	...
	// Inputs
	public int _threadnum, _number_threads;
	private byte[] _privateKey,  _randomness, _randomnessInput;
	private int[] _A;
	...
	InnerSigningThread(int tn, int nt, CountDownLatch countDownLatch, byte[] privateKey,
			byte[] randomness, byte[] randomnessInput, int[] A,
			QTESLAYSampler qTESLAYSampler2, QTESLAParameter parameter2, Polynomial polynomial2) {
		...
		_randomnessInput = randomnessInput;
		_A = A;
	...
	public void run () 	
	{
		...
		while (running.get()) {
		    ...
		    for (int k=0; k<parameter.k; k++) {
		    	polynomial.polynomialMultiplication_MB(V, k*parameter.n, _A, k*parameter.n, Y_ntt, 0);
		    }	    
		    
		    for(int g=0; g<C.length; g++) C[g]=0;
		    hashFunction_MB (C, V, _randomnessInput, QTESLAParameter.RANDOM + QTESLAParameter.SEED);
		...
		//System.out.println("Thread " + _threadnum + " finishes after " + cnt + ".(" + response + ")");
		QTESLA.inner_done();
