...
public class QTESLA {
	...
	private static InnerSigningThread inner_winner = null;
...
synchronized static void inner_done()
{
    // Threads calling this are going to be RaceHorse objects.
    // Now, if there isn't already a winner, this RaceHorse is the winner.
	{
	    if (inner_winner == null) {
	    	inner_winner = (InnerSigningThread) Thread.currentThread();
	    	//System.out.println("Done() called by " + inner_winner.getName());
	    }
	}
    
}

synchronized static boolean is_inner_done()
{
    // Threads calling this are going to be RaceHorse objects.
    // Now, if there isn't already a winner, this RaceHorse is the winner.
    if (inner_winner == null) {
	...
	{		
		...
		byte[] randomness				= new byte[QTESLAParameter.SEED];
		byte[] randomnessInput			= new byte[144];	
		int[] A		= new int[parameter.n * parameter.k];	
		//System.out.println(Arrays.toString(message));
		...
		CountDownLatch countDownLatch = new CountDownLatch(1);	
		ArrayList<InnerSigningThread> threadies = new ArrayList<InnerSigningThread>();
		
		for (int i=0; i<number_threads;i++) {	
			InnerSigningThread t1 = new InnerSigningThread(i, number_threads, countDownLatch, 
					privateKey, randomness, randomnessInput, A,
					qTESLAYSampler, parameter, polynomial);	
			threadies.add(	t1	);
			threadies.get(i).start();
			//threadies.get(i).join();
		}
		...
		countDownLatch.await(); 
		/* Pack Signature */
		qTESLAPack.encodeSignature_MB (signature, 0, inner_winner.C, 0, inner_winner.Z);		

		for (int i=0; i<number_threads;i++) {	
			//System.out.println("Stopping " + i);
			threadies.get(i).stopMe();			
		}
	...
	public int verify_MB (
		...
		//System.out.println("3. " + Arrays.toString(signature));
